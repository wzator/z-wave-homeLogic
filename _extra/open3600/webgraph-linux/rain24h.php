...
while (!feof($fil)) {
    ...
    if (IsSet($components[13]))
    {
        $time[$i]= $components[2];
        $date[$i]= $components[1];
        $value1[$i]= $components[13];
        $i++;
    }    
}
fclose($fil);
...
$line->parameter['title'] = 'Rain 24 hours';
