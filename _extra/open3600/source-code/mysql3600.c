...
//        int tempint_min, tempint_max;
  ...
  if (read_result == -1)
	...
	if (temperature_outdoor(data) > 60)
	{
	    sprintf(logline,"%s\NULL,", logline);
	}
	else
	{
	    sprintf(logline,"%s\'%.1f\',", logline,
	...
	// printf("%s%s",datestring, logline);
        ...
        sprintf(query, "INSERT INTO data VALUES (%s,%s)", datestring, logline);
